...
#     if bands_only: img = img[:,:,:num_bands] # Original
#     #if bands_only: img = img[:,:]
#     return img
...
def get_tiles(tile_dir, img_dir, img_triplets, tile_size=50, neighborhood=100,
                      val_type='uint8', bands_only=False, save=True, verbose=False):
    if not os.path.exists(tile_dir):
        os.makedirs(tile_dir)
    size_even = (tile_size % 2 == 0)
    ...
    n_triplets = img_triplets.shape[0]
    print("n_triplets =  {}".format(n_triplets))
    unique_imgs = np.unique(img_triplets)
    tiles = np.zeros((n_triplets, 3, 2), dtype=np.int16)
    ...
    for img_name in unique_imgs:
        ...
        img_padded = np.pad(img, pad_width=[(tile_radius, tile_radius),
                            ...
                            mode='reflect')
        img_shape = img_padded.shape
        print("img_padded.shape =  {}".format(img_padded.shape))
...
def get_label(tiles_dir, verbose=False):  # 特征提取
    ...
    for i in range(tiles_num):
        if verbose:
            print("get label of tile {}".format(tiles[i]))
        img_name = tiles_dir + "/" + tiles[i]
        img = np.load(img_name)
        ground_truth = img[:,:,-1]
        h, w = ground_truth.shape
#         print(ground_truth.shape)
        counts_gt = np.bincount(ground_truth.reshape(int(h*w),))
        if verbose:
            print(ground_truth.reshape(int(h*w),).shape)
        label = np.argmax(counts_gt)
        label_total.append(label)
    np.save(os.path.join(tiles_dir, 'y.npy'), np.array(label_total))
    return np.array(label_total)
...
def npy2jpg(dir,dest_dir):
    ...
    for i in range(5):
        tile = dir + str(i) + 'tile.npy'
        tiledata = np.load(tile)
        tiledata = tiledata[:,:,0:3]
        #plt.imshow(anchor[:,:,0:3])
        img_name = dest_dir + str(i)+"tile.jpg"
        matplotlib.image.imsave(img_name, tiledata)
        #img.save(dest_dir+str(i+1)+"tile.jpg")